...
- **Admin decisions**: Approve is the _only_ solid-filled semantic button in the product — reserved because it's the one action here with an immediate, public, irreversible effect (grants the owner role, makes phone public). Reject requires a note; the note is shown verbatim on the applicant's status screen (2o).
